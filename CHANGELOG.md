...
### Fixed
- `POSITIONAL` (2) output function: `channelDescriptions`, `channelSettings`, and `channelStates` container keys now use the **ChannelType integer string** (e.g. `"7"` for `shadePositionOutside`, `"8"` for `shadePositionIndoor`, `"9"` for `shadeOpeningAngleOutside`, `"10"` for `shadeOpeningAngleIndoor`) instead of the channel name. The vdSM parses the outer key as a `ChannelType` integer to build the OPC table; non-numeric keys are silently skipped → empty OPC → `m_emulatedHardwareOutputs = true` for grey GR-class devices → bus errors (`DSMeter Error [1] wrong parameter class_:64 index:2/4`). Using the ChannelType integer fixes the OPC table and routes all shade commands through the VDC TCP path.
- `setOutputChannelValue` and `dimChannel` handlers in `VdcHost` now resolve `channelId` by both channel name **and** ChannelType integer string (e.g. `"7"`), so incoming shade channel notifications are correctly dispatched.
- Removed S2 awning workaround in `examples/example_shading.py` that forced `name="0"` to paper over the POSITIONAL key bug; the example now uses the standard `add_channel(OutputChannelType.SHADE_POSITION_OUTSIDE)` call.
