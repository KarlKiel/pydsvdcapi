...
from full_showcase import build_all_devices, VENDOR_NAME, STATE_FILE
...
_GROUP_TITLE_FILL = {
    1: "FFD700", 2: "C0C0C0", 3: "87CEEB", 4: "E0FFFF",
    5: "EE82EE", 6: "FF6B6B", 7: "90EE90", 8: "A9A9A9",
    9: "F0F0F0", 10: "B0E0E6", 11: "ADD8E6", 12: "87CEEB",
    48: "AFEEEE", 64: "E0FFFF", 65: "D3D3D3",
}
...
SCHEMA_IDENT_VDSD = [
    ...
    ("hardwareGuid",       "Optional*",  "Unique instance ID mac-address:XX:…; required for GTIN and name distribution"),
    ("hardwareModelGuid",  "Optional*",  "Model-class ID ean:(01)…; required for GTIN and name distribution"),
    ...
    ("oemModelGuid",       "Optional",   "GTIN / OEM model GUID gs1:(01)…; enables hasActions in dSS VdcDb"),
    ...
    ("primaryGroup",       "Required",   "Device colour / application type (ColorClass enum integer)"),
...
SCHEMA_VDC_CAPABILITIES = [
    ...
    ("identification",     "Optional",   "Device identification / blink support (default: false)"),
    ("dynamicDefinitions", "Optional",   "dSS queries VDC live for states/events/actions (default: false)"),
]

SCHEMA_MODEL_FEATURES = []   # handled specially — just a flat set of names

SCHEMA_OUTPUT_DESC = [
    ("function",           "Required",   "OutputFunction: 0=ON_OFF 1=POSITIONAL 2=DIMMER 3=DIMMER_COLOR_TEMP 4=FULL_COLOR_DIMMER 5=INTERNALLY_CONTROLLED 6=BIPOLAR 255=CUSTOM"),
    ...
    ("activeCoolingMode",  "Optional",   "True for dual heat/cool outputs (climate devices)"),
]
...
SCHEMA_OUTPUT_SETTINGS = [
    ...
    ("pushChanges",        "Required",   "Push value changes to dSS proactively (true recommended)"),
    ...
    ("heatingSystemCapability", "Optional", "HeatingSystemCapability enum — required for climate outputs"),
    ("heatingSystemType",  "Optional",   "HeatingSystemType enum — required for climate outputs"),
]
...
SCHEMA_BTN_DESC = [
    ...
    ("buttonType",         "Required",   "ButtonType: 0=UNDEFINED 1=SINGLE_PUSHBUTTON 2=TWO_WAY 3=FOUR_WAY_NAV 4=FOUR_WAY_CENTER 6=ON_OFF_SWITCH"),
    ("buttonElementID",    "Required",   "ButtonElementID: 0=CENTER 1=DOWN 2=UP 3=LEFT 4=RIGHT …"),
...
SCHEMA_BTN_SETTINGS = [
    ...
    ("setsLocalPriority",  "Optional",   "Whether pressing sets local priority (default: false)"),
    ("callsPresent",       "Optional",   "Whether pressing sends 'present' call (default: false)"),
]
...
SCHEMA_BTN_STATE = [
    ...
    ("actionId",           "Optional",   "Action ID — used in action mode instead of value/clickType"),
...
SCHEMA_BINARY_DESC = [
    ...
    ("sensorFunction",     "Required",   "BinaryInputType enum (hardwired sensor classification)"),
...
SCHEMA_SENSOR_SETTINGS = [
    ...
    ("changesOnlyInterval","Optional",   "Push on change only within this interval (seconds)"),
]
...
def _write_title_row(ws, row: int, text: str,
                     fill_color: str = C_DEV_TITLE, ncols: int = 5) -> int:
...
def _write_col_headers(ws, row: int,
                       labels: tuple = ("Property", "Sub-key / Index",
                                        "Value", "Req / Opt", "API Note")) -> int:
...
def _write_schema_rows(ws, row: int, schema: list,
                       actual: dict, indent: int = 0) -> int:
...
def _write_nested_schema_rows(ws, row: int, outer_key: str, schema: list,
                               actual: dict) -> int:
...
def build_vdc_sheet(wb: openpyxl.Workbook, vdc_props: dict) -> None:
    ...
    row = _write_nested_schema_rows(ws, row, "capabilities", SCHEMA_VDC_CAPABILITIES, caps_actual)
    row += 1
...
def build_vdsd_sheet(wb: openpyxl.Workbook, di_idx: int, name: str,
                     props: dict, primary_group: int) -> None:
    ...
    if mf:
        ...
        for i, feat in enumerate(all_features):
            ...
            nc = ws.cell(row=row, column=5, value="One entry per enabled feature flag (boolean true)")
            nc.font = Font(size=8, italic=True, color="444444")
            nc.fill = _fill(val_bg)
            nc.border = _border()
            nc.alignment = _align()
        ...
        row = _write_schema_rows(ws, row, [("modelFeatures", "Optional", "Dict of enabled feature flags {featureName: true}")], props)
    row += 1
    ...
    if "outputDescription" in props or "outputSettings" in props:
        ...
        row = _write_nested_schema_rows(ws, row, "outputDescription", SCHEMA_OUTPUT_DESC, out_desc)
        row = _write_nested_schema_rows(ws, row, "outputSettings",    SCHEMA_OUTPUT_SETTINGS, out_set)
        row = _write_nested_schema_rows(ws, row, "outputState",       SCHEMA_OUTPUT_STATE, out_st)
        row += 1
    ...
    if ch_desc or ch_set or ch_state:
        ...
        for idx_str in all_idx:
            ...
            row = _write_nested_schema_rows(ws, row, "  description", SCHEMA_CHANNEL_DESC,  ch_desc.get(idx_str, {}))
            if SCHEMA_CHANNEL_SETTINGS:
                row = _write_nested_schema_rows(ws, row, "  settings", SCHEMA_CHANNEL_SETTINGS, ch_set.get(idx_str, {}))
            row = _write_nested_schema_rows(ws, row, "  state",       SCHEMA_CHANNEL_STATE, ch_state.get(idx_str, {}))
        row += 1
    ...
    if scenes:
        ...
        preview = ", ".join(nums[:12]) + (f"  … ({len(nums)} total)" if len(nums) > 12 else "")
        ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=2)
        ka = ws.cell(row=row, column=1, value="scene numbers present")
        ka.font = Font(size=9); ka.fill = _fill(C_KEY); ka.border = _border(); ka.alignment = _align()
        vc = ws.cell(row=row, column=3, value=preview)
        vc.font = Font(size=9); vc.fill = _fill(C_VAL_A); vc.border = _border(); vc.alignment = _align()
        dc = ws.cell(row=row, column=4, value="Required")
        dc.font = Font(size=9, bold=True); dc.fill = _fill(C_REQ); dc.border = _border()
        dc.alignment = Alignment(horizontal="center", vertical="top")
        nc = ws.cell(row=row, column=5, value="Scene value table — Required when output has channels")
        nc.font = Font(size=8, italic=True, color="444444"); nc.fill = _fill(C_VAL_A); nc.border = _border(); nc.alignment = _align()
        row += 1
        row += 1
    ...
    if btn_desc or btn_set or btn_st:
        ...
        for idx_str in all_idx:
            ...
            row = _write_nested_schema_rows(ws, row, "  description", SCHEMA_BTN_DESC,     btn_desc.get(idx_str, {}))
            row = _write_nested_schema_rows(ws, row, "  settings",    SCHEMA_BTN_SETTINGS, btn_set.get(idx_str, {}))
            row = _write_nested_schema_rows(ws, row, "  state",       SCHEMA_BTN_STATE,    btn_st.get(idx_str, {}))
        row += 1
    ...
    if bi_desc or bi_set or bi_st:
        ...
        for idx_str in all_idx:
            ...
            row = _write_nested_schema_rows(ws, row, "  description", SCHEMA_BINARY_DESC,     bi_desc.get(idx_str, {}))
            row = _write_nested_schema_rows(ws, row, "  settings",    SCHEMA_BINARY_SETTINGS, bi_set.get(idx_str, {}))
            row = _write_nested_schema_rows(ws, row, "  state",       SCHEMA_BINARY_STATE,    bi_st.get(idx_str, {}))
        row += 1
    ...
    if si_desc or si_set or si_st:
        ...
        for idx_str in all_idx:
            ...
            row = _write_nested_schema_rows(ws, row, "  description", SCHEMA_SENSOR_DESC,     si_desc.get(idx_str, {}))
            row = _write_nested_schema_rows(ws, row, "  settings",    SCHEMA_SENSOR_SETTINGS, si_set.get(idx_str, {}))
            row = _write_nested_schema_rows(ws, row, "  state",       SCHEMA_SENSOR_STATE,    si_st.get(idx_str, {}))
        row += 1
    ...
    if act_desc is not None:
        row = _write_section_header(ws, row, "ACTION DESCRIPTIONS  (SingleDevice §4.5.2)")
        if act_desc:
            for name_key, entry in act_desc.items():
                row = _write_index_banner(ws, row, f"Action  '{name_key}'")
                row = _write_nested_schema_rows(ws, row, "  description", SCHEMA_ACTION_DESC, entry if isinstance(entry, dict) else {})
        else:
            row = _write_schema_rows(ws, row, [("(empty — no actions)", "Required", "Must be present for SingleDevice even if empty")], {})
        row += 1
    ...
    if std_act is not None:
        ...
        if std_act:
            for name_key, entry in std_act.items():
                ...
                row = _write_nested_schema_rows(ws, row, "  properties", SCHEMA_STD_ACTION, entry if isinstance(entry, dict) else {})
        else:
    ...
    if ev_desc is not None:
        row = _write_section_header(ws, row, "EVENT DESCRIPTIONS  (SingleDevice §4.7.1)")
        if ev_desc:
            for name_key, entry in ev_desc.items():
                row = _write_index_banner(ws, row, f"Event  '{name_key}'")
                row = _write_nested_schema_rows(ws, row, "  description", SCHEMA_EVENT_DESC, entry if isinstance(entry, dict) else {})
        else:
            row = _write_schema_rows(ws, row, [("(empty)", "Required", "Must be present for SingleDevice")], {})
        row += 1
    ...
    if st_desc is not None:
        row = _write_section_header(ws, row, "STATE DESCRIPTIONS + VALUES  (SingleDevice §4.6)")
        if st_desc:
            for name_key, entry in st_desc.items():
                row = _write_index_banner(ws, row, f"State  '{name_key}'")
                row = _write_nested_schema_rows(ws, row, "  description", SCHEMA_STATE_DESC, entry if isinstance(entry, dict) else {})
                row = _write_nested_schema_rows(ws, row, "  current value", SCHEMA_STATE_VAL, st_vals.get(name_key, {}))
        else:
    ...
    if pr_desc is not None:
        row = _write_section_header(ws, row, "PROPERTY DESCRIPTIONS + VALUES  (SingleDevice §4.6)")
        if pr_desc:
            for name_key, entry in pr_desc.items():
                current_val = pr_vals.get(name_key, _SENTINEL)
                row = _write_index_banner(ws, row, f"Property  '{name_key}'  =  {_format_val(current_val)}")
                row = _write_nested_schema_rows(ws, row, "  description", SCHEMA_PROP_DESC, entry if isinstance(entry, dict) else {})
        else:
            row = _write_schema_rows(ws, row, [("(empty)", "Required", "")], {})
        row += 1
